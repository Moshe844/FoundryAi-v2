...
test("compiled locators are exact, because Playwright's are not", () => {
  // A build died here: getByLabel("New task") also matches "New task progress",
  // so the strict action aborted and the bundle gate refused the file Foundry
  ...
  assert.match(source, /getByLabel\("New task", \{ exact: true \}\)/u);
  assert.match(source, /getByRole\("button", \{ name: "Add task", exact: true \}\)/u);
});

test("the compiled module announces itself, so Foundry does not police its own code", () => {
  // The model-code rules exist to police how a model chose to write Playwright.
  // Run against a file Foundry generated, they refuse a bundle over Foundry's
  // own style.
  const source = compileDeclaredChecks(
    declare([{ checkId: "a", primitive: P.ELEMENT_VISIBLE, target: rows }]),
  );
  assert.match(source, /^\/\/ Generated by Foundry/u);
});
