...
export function validateProjectBundleForStack(
  ...
  // Foundry's own compiled modules are exempt from the model-code rules. Those
  // rules police how a model chose to write Playwright; applying them to a file
  // Foundry generated means refusing a bundle over Foundry's own style, and a
  // build died exactly that way -- the compiled runner used getByLabel without
  // exact matching. The compiler is where that belongs, and it now emits exact
  // locators.
  const browserTests = [...byPath.entries()]
    .filter(
      ([path, content]) =>
        path.startsWith("tests/") &&
        !/^\/\/ Generated by Foundry/u.test(String(content)),
    )
    .map(([, content]) => content)
    .join("\n");
